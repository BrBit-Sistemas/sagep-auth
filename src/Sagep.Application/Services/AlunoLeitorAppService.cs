            ...
            try
            {
                ...
                if (alunoLeitor != null)
                {
                    if (alunoLeitor.TenantId == tenantId && alunoLeitor.IsDeleted == false)
                    {
                        ...
                        if (alunoLeitor.TenantId != tenantId && alunoLeitor.IsDeleted == false)
                        {
                            _validationResult.AddErrorMessage($"Aluno Leitor já cadastrado e ativo na UNIDADE PRISIONAL {alunoLeitor.Tenant.NomeExibicao}. <br>Solicite a UNIDADE PRISIONAL a inativação do leitor e tente novamente.");
                            return _validationResult;
                        }

                        alunoLeitor.IsDeleted = false;
