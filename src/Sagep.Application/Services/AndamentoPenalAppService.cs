...
using Sagep.Application.Resolvers;
    ...
    public class AndamentoPenalAppService : IAndamentoPenalAppService
    {
        ...
        public ValidationResult Add(AndamentoPenalViewModel andamentoPenalViewModel)
        {
            try
            {
                ...
                Guid tenantId;
                if (!TenantResolver.TryGetTenantIdAsync(out tenantId)) _validationResult.AddErrorMessage("Problemas ao obter os dados da UNIDADE PRISIONAL do usuário logado.");
                #endregion
                ...
                if (detentoAndamentoPenal == null)
                {
                    ...
                    andamentoPenalMapped.TenantId = tenantId;
                ...
                {
                    if (!detentoAndamentoPenal.IsDeleted && detentoAndamentoPenal.TenantId != tenantId)
                    {
                        _validationResult.AddErrorMessage($"Já existe um registro ativo no andamento penal da UNIDADE PRISIONAL {detentoAndamentoPenal.Tenant.NomeExibicao}. <br> Solicite a UNIDADE para inativá-lo e tente novamente.");
                        return _validationResult;
                    }
                    else if (!detentoAndamentoPenal.IsDeleted && detentoAndamentoPenal.TenantId == tenantId)
                    {
                        _validationResult.AddErrorMessage($"Já existe um registro ativo no andamento penal da sua UNIDADE PRISIONAL.");
                        return _validationResult;
                        
                    } else {
                        andamentoPenalViewModel.IsDeleted = false;
                        andamentoPenalViewModel.DetentoId = detentoAndamentoPenal.DetentoId;
                        andamentoPenalViewModel.Id = detentoAndamentoPenal.Id;
                        andamentoPenalViewModel.TenantId = tenantId;
                        detentoAndamentoPenal.Tenant = null;
        ...
        public ValidationResult Update(AndamentoPenalViewModel andamentoPenalViewModel)
        {
            try
            {
                ...
                andamentoPenalViewModel.TenantId = apDB.TenantId;
                var apMapper = _mapper.Map<AndamentoPenalViewModel, AndamentoPenal>
